...
LightUI:SetScript("OnEvent", function(self, event)
    ...
    print(
        "Welcome to |cfff58cbaLight|r|cff009cffUI|r use |CFFFE8A0E/LightUI|r for Options.")

    SlashCmdList['LIGHTUICMD'] = function(msg)
        LightUIDB = nil
        ReloadUI()
    end
    SLASH_LIGHTUICMD1 = '/reset'
    ...
    local defaults = {
        ...
        FONTS = {
            NORMAL = "Interface\\AddOns\\" .. A ..
                "\\media\\fonts\\FRIZQT__.TTF"
        },
        UNITFRAMES = {
            ACTIV = true,
            SIZE = 1,
        ...
        textures = {
            ...
            buttonback = "Interface\\AddOns\\" .. A ..
                "\\media\\core\\button_background",
            buttonbackflat = "Interface\\AddOns\\" .. A ..
                "\\media\\core\\button_background_flat",
            outer_shadow = "Interface\\AddOns\\" .. A ..
                "\\media\\core\\outer_shadow"
        },
        background = {
...
local function LightUICreateConfig()
    -- FRAME
    local LightUIConfig = CreateFrame("Frame", "LightUIConfig", UIParent,
                                      "ButtonFrameTemplate")
    SetPortraitToTexture(LightUIConfig.portrait,
    ...
    LightUIConfig:SetScript("OnDragStop",
                            function(self) self:StopMovingOrSizing() end)
    LightUIConfig:Hide()

    local LightUIConfigTitle = LightUIConfig:CreateFontString(nil,
                                                              LightUIConfig,
                                                              "GameFontNormalLarge")
    ...
    -- COLOR
    for i, v in pairs({LightUIConfigBg, LightUIConfigTitleBg}) do
        v:SetVertexColor(.40, .40, .40)
    end

    for i, v in pairs({LightUIConfigPortraitFrame}) do
...
local function LightUICONFIGSHOW()
    ...
    local function RESET()
        StaticPopupDialogs["RESET"] = {
            ...
            button1 = "Close",
            button2 = "Reset UI",
            OnAccept = function() LightUICONFIGSHOW() end,
            OnCancel = function()
                LightUIDB = nil
                ReloadUI()
            end,
    ...
    -- LightUI BUTTONS
    local function LightUICreateBTN(text, point1, anchor, point2, pos1, pos2,
                                    width, height)
        LightUIBTN = CreateFrame("Button", nil, LightUIConfig,
    ...
    local function LightUICreateTXT(text, anchor, point, pos1, pos2, ...)
        LightUITXT = LightUIConfig:CreateFontString(nil, LightUIConfig,
                                                    "GameFontNormalLarge")
    ...
    local function LightUICreateCB(name, anchor, tooltip, db, ...)
        LightUICheckbox = CreateFrame("CheckButton", nil, LightUIConfig,
                                      "OptionsBaseCheckButtonTemplate")
        LightUICheckbox:SetPoint("TOPLEFT", anchor, "BOTTOMLEFT", 0, -5)

        LightUICheckbox:SetScript("OnClick", function(frame)
        ...
        LightUICheckbox:SetScript("OnShow", function(frame)
            frame:SetChecked(LightUIDB[db])
        end)

        LightUICheckbox:SetScript("OnEnter", function(self)
        ...
        text = LightUIConfig:CreateFontString(nil, LightUIConfig,
                                              "GameFontHighlight")
    ...
    -- BUTTONS
    LightUICreateBTN("Save & Reload", "BOTTOMLEFT", LightUIConfig, "BOTTOMLEFT",
                     16, 35, 100, 25)
    LightUIBTN:SetScript("OnClick", function(self, button, down) ReloadUI() end)

    LightUICreateBTN("Reset", "BOTTOMLEFT", LightUIConfig, "BOTTOMLEFT", 120,
                     35, 100, 25)
    LightUIBTN:SetScript("OnClick", function(self, button, down)
        LightUIConfig:Hide()
        RESET()
    end)
    ...
    LightUICreateBTN("Edit", "BOTTOMLEFT", LightUIConfig, "BOTTOMRIGHT", -116,
                     35, 100, 25)
    LightUIBTN:SetScript("OnClick", function(self, button, down)
        LightUIConfig:Hide()
        LightUIEDIT()
    ...
    LightUICreateCB("Unitframes", LightUITXT, "Custom UnitFrames",
                    "A_UNITFRAMES")
    LightUICreateCB("PartyFrames", LightUICheckbox, "Custom PartyFrames",
                    "A_PARTYFRAMES")
    ...
    LightUICreateCB("Macros", LightUICheckbox, "Show Macros on Actionbar",
                    "A_MACROS")
    LightUICreateCB("Gryphones", LightUICheckbox, "Show Gryphones",
                    "A_GRYPHONES")
    LightUICreateCB("Dark Frames", LightUICheckbox, "Dark Interface",
                    "A_DARKFRAMES")
    LightUICreateCB("Friendlist", LightUICheckbox, "Friendlist Class Colors",
                    "A_FRIENDLIST")
    ...
    LightUICreateCB("Range", LightUICheckbox, "Actionbar Spellrange Color",
                    "A_RANGE")
    LightUICreateCB("Invismenu", LightUICheckbox, "Invisible Mainmenu",
                    "A_INVISBAGS")
    ...
    LightUICreateCB("Safequeue", LightUITXT, "Time left for Q accept",
                    "A_SAFEQUEUE")
    LightUICreateCB("SmartKitty", LightUICheckbox,
                    "Action bar for display druid dps cycle", "A_SMARTKITTY")
    LightUICreateCB("TabBinder", LightUICheckbox,
                    "Binds the Tabkey to Player only in Arena", "A_TABBINDER")
    LightUICreateCB("KeyPress", LightUICheckbox, "Keypress Effect on Actionbar",
                    "A_KEYPRESS")
    ...
    LightUICreateCB("ShowViewport", LightUITXT, "Show top and bottom Viewports",
                    "A_VIEWPORT")
    LightUICreateCB("InCombat", LightUICheckbox,
                    "Incombat Symbols on UnitFrames", "A_INCOMBAT")
    LightUICreateCB("Charlinks", LightUICheckbox, "Charlinks on UnitFrame Menu",
                    "A_CHARLINKS")
    LightUICreateCB("HoverBind", LightUICheckbox,
                    "/hb to bind Keys on Actionbar", "A_HOVERBIND")
    LightUICreateCB("TalkingHead", LightUICheckbox,
                    "Show Talking Head over Actionbar", "A_TALKINGHEAD")
    ...
    LightUICreateCB("EasyDelete", LightUICheckbox, "Easy delete Items",
                    "A_EASYDELETE")
    LightUICreateCB("OrderhallBar", LightUICheckbox, "Show the Orderhallbar",
                    "A_ORDERHALLBAR")
    -- END
