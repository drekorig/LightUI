...
if not LightUIDB.A_DARKFRAMES == true then return end
...
--rewrite the oneletter shortcuts
  if LightUIDB.adjustOneletterAbbrev then
...
--backdrop debuff
  local backdropDebuff = {
    bgFile = nil,
    edgeFile = LightUIDB.debuffFrame.background.edgeFile,
    tile = false,
    tileSize = 32,
    edgeSize = LightUIDB.debuffFrame.background.inset,
    insets = {
      left = LightUIDB.debuffFrame.background.inset,
      right = LightUIDB.debuffFrame.background.inset,
      top = LightUIDB.debuffFrame.background.inset,
      bottom = LightUIDB.debuffFrame.background.inset,
    },
  }
...
--backdrop buff
  local backdropBuff = {
    bgFile = nil,
    edgeFile = LightUIDB.buffFrame.background.edgeFile,
    tile = false,
    tileSize = 32,
    edgeSize = LightUIDB.buffFrame.background.inset,
    insets = {
      left = LightUIDB.buffFrame.background.inset,
      right = LightUIDB.buffFrame.background.inset,
      top = LightUIDB.buffFrame.background.inset,
      bottom = LightUIDB.buffFrame.background.inset,
    },
  }
...
--apply aura frame texture func
  local function applySkin(b)
    ...
    if debuff then
      LightUI = LightUIDB.debuffFrame
      backdrop = backdropDebuff
    else
      LightUI = LightUIDB.buffFrame
      backdrop = backdropBuff
    end

--check class coloring options
    --button
    b:SetSize(LightUI.button.size, LightUI.button.size)
    ...
    icon:ClearAllPoints()
    icon:SetPoint("TOPLEFT", b, "TOPLEFT", -LightUI.icon.padding, LightUI.icon.padding)
    icon:SetPoint("BOTTOMRIGHT", b, "BOTTOMRIGHT", LightUI.icon.padding, -LightUI.icon.padding)
    icon:SetDrawLayer("BACKGROUND",-8)
    b.icon = icon

    --border
    local border = _G[name.."Border"] or b:CreateTexture(name.."Border", "BACKGROUND", nil, -7)
    border:SetTexture(LightUI.border.texture)
    border:SetTexCoord(0,1,0,1)
    border:SetDrawLayer("BACKGROUND",-7)
    if tempenchant then
      border:SetVertexColor(0.7,0,1)
    elseif not debuff then
      border:SetVertexColor(LightUI.border.color.r,LightUI.border.color.g,LightUI.border.color.b)
    end
    ...
    --duration
    b.duration:SetFont(FONT, LightUI.duration.size, "THINOUTLINE")
    b.duration:ClearAllPoints()
    b.duration:SetPoint(LightUI.duration.pos.a1,LightUI.duration.pos.x,LightUI.duration.pos.y)

    --count
    b.count:SetFont(LightUI.count.font, LightUI.count.size, "THINOUTLINE")
    b.count:ClearAllPoints()
    b.count:SetPoint(LightUI.count.pos.a1,LightUI.count.pos.x,LightUI.count.pos.y)

    --shadow
    if LightUI.background.show then
      local back = CreateFrame("Frame", nil, b)
      back:SetPoint("TOPLEFT", b, "TOPLEFT", -LightUI.background.padding, LightUI.background.padding)
      back:SetPoint("BOTTOMRIGHT", b, "BOTTOMRIGHT", LightUI.background.padding, -LightUI.background.padding)
      back:SetFrameLevel(b:GetFrameLevel() - 1)
      back:SetBackdrop(backdrop)
      back:SetBackdropBorderColor(LightUI.background.color.r,LightUI.background.color.g,LightUI.background.color.b,LightUI.background.color.a)
      b.bg = back
    end
...
--update debuff anchors
  local function updateDebuffAnchors(buttonName,index)
    ...
    if index == 1 then
      if LightUIDB.combineBuffsAndDebuffs then
    ...
    elseif index > 1 and mod(index, LightUIDB.debuffFrame.buttonsPerRow) == 1 then
      button:SetPoint("TOPRIGHT", _G[buttonName..(index-LightUIDB.debuffFrame.buttonsPerRow)], "BOTTOMRIGHT", 0, -LightUIDB.debuffFrame.rowSpacing)
    else
      button:SetPoint("TOPRIGHT", _G[buttonName..(index-1)], "TOPLEFT", -LightUIDB.debuffFrame.colSpacing, 0)
    end
  end
...
--update buff anchors
  local function updateAllBuffAnchors()
   ...
   -- if ShouldShowConsolidatedBuffFrame() then
     ...
     -- TempEnchant1:SetPoint("TOPRIGHT", ConsolidatedBuffs, "TOPLEFT", -LightUIDB.buffFrame.colSpacing, 0)
      --offset = offset + 1
   -- else
      TempEnchant1:ClearAllPoints()
   ...
   -- if ShouldShowConsolidatedBuffFrame() then
    ...
    for index = 1, numBuffs do
      ...
      if not button.consolidated then
        ...
        if realIndex == 1 then
          ...
          aboveButton = button
        elseif realIndex > 1 and mod(realIndex, LightUIDB.buffFrame.buttonsPerRow) == 1 then
          button:SetPoint("TOPRIGHT", aboveButton, "BOTTOMRIGHT", 0, -LightUIDB.buffFrame.rowSpacing)
          aboveButton = button
        else
          button:SetPoint("TOPRIGHT", previousButton, "TOPLEFT", -LightUIDB.buffFrame.colSpacing, 0)
        end
    ...
    local rows = ceil((buffCounter+offset)/LightUIDB.buffFrame.buttonsPerRow)
    local height = LightUIDB.buffFrame.button.size*rows + LightUIDB.buffFrame.rowSpacing*rows + LightUIDB.buffFrame.gap*min(1,rows)
    buffFrameHeight = height
    --make sure the debuff frames update the position asap
    if DebuffButton1 and LightUIDB.combineBuffsAndDebuffs then
...
--buff drag frame
  ...
  bf:SetSize(LightUIDB.buffFrame.button.size,LightUIDB.buffFrame.button.size)
  bf:SetPoint(LightUIDB.buffFrame.pos.a1,LightUIDB.buffFrame.pos.af,LightUIDB.buffFrame.pos.a2,LightUIDB.buffFrame.pos.x,LightUIDB.buffFrame.pos.y)
  if LightUIDB.buffFrame.userplaced then
    rCreateDragFrame(bf, dragFrameList, -2 , true) --frame, dragFrameList, inset, clamp
  end

  if not LightUIDB.combineBuffsAndDebuffs then

--debuff drag frame
  local df = CreateFrame("Frame", "rBFS_DebuffDragFrame", UIParent)
  df:SetSize(LightUIDB.debuffFrame.button.size,LightUIDB.debuffFrame.button.size)
  df:SetPoint(LightUIDB.debuffFrame.pos.a1,LightUIDB.debuffFrame.pos.af,LightUIDB.debuffFrame.pos.a2,LightUIDB.debuffFrame.pos.x,LightUIDB.debuffFrame.pos.y)
  if LightUIDB.debuffFrame.userplaced then
  ...
  TempEnchant2:SetPoint("TOPRIGHT", TempEnchant1, "TOPLEFT", -LightUIDB.buffFrame.colSpacing, 0)
  TempEnchant3:ClearAllPoints()
  TempEnchant3:SetPoint("TOPRIGHT", TempEnchant2, "TOPLEFT", -LightUIDB.buffFrame.colSpacing, 0)
  ...
  --ConsolidatedBuffsTooltip:SetScale(LightUIDB.consolidatedTooltipScale)
